...
var JobManager = module.exports = function JobManager(options) {
    ...
    this.processShare = async function (jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, workerPassword, soln) {
        ...
        // Initialize worker data in global_worker_dict if not present
        if (!global_worker_dict[pslAddress]) {
            global_worker_dict[pslAddress] = {
                remote_ip: remoteAPIValidatorIp,
                authToken: authToken,
                can_accept_mining_workers_shares_currently: false, // Default to false until validated
        ...
        // Access the worker's data to get the merkle root
        const workerData = this.globalWorkerDict[pslAddress]; // pslAddress should be replaced with the correct worker identifier
        const previousBlockMerkleRoot = workerData ? workerData.previous_block_merkle_root : null;

        if (!previousBlockMerkleRoot) {
...
JobManager.prototype.updateWorkerData = function(workerId, update) {
    this.globalWorkerDict = this.globalWorkerDict || {};
    this.globalWorkerDict[workerId] = update;

    console.log(`JobManager updated worker ${workerId} data: `, update);
};