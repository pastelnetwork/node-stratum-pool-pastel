...
var JobManager = (module.exports = function JobManager(options) {
  ...
  this.processShare = async function (
    ...
    const previousBlockMerkleRoot = workerData
      ? workerData.previous_block_merkle_root
      : null;
    ...
    console.log(
      "Processing Share Details:\n" +
        ...
        "  - nTime: " +
        nTime +
        " (The time when the miner started working on this particular job)\n" +
        "  - Nonce: " +
        nonce +
        " (The nonce value that was successful in solving the block)\n" +
        "  - IP Address: " +
        ipAddress +
        "\n" +
        ...
        "  - PSL Address: " +
        pslAddress +
        " (The address in the PSL (Pool Share Ledger) crediting this share)\n" +
        "  - Solution: " +
        soln +
        " (The solution to the block that was found)"
    );
    ...
    var headerSolnBuffer = new Buffer.concat([
      headerBuffer,
    ...
    if (headerBigNum.le(job.target)) {
      ...
      blockHex = job
        .serializeBlock(headerBuffer, Buffer.from(soln, "hex"))
        .toString("hex");
