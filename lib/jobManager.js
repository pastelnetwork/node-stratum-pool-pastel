...
var JobManager = (module.exports = function JobManager(options) {
  ...
  this.processShare = async function (
    ...
    const headerDataParts = job.serializeHeaderData(nTime, refreshedWorkerData);
    const headerSolnBuffer = job.serializeBlockHeader(headerDataParts, nonce, soln);
    const equihashInput = job.serializeEquihashInput(headerDataParts, nonce);
    ...
    if (headerBigNum.le(job.target)) {
      ...
      blockHex = job.serializeBlock(headerSolnBuffer).toString("hex");
    ...
    await validateShareAsync(shareData, shareId, _this, blockHex, blockHash); // Pass the context (_this) if necessary
    }
