const bitcoin = require("@pastelnetwork/psl-utxo-lib");
...
const scriptFoundersCompile = (address) =>
  bitcoin.script.compile([
    bitcoin.opcodes.OP_HASH160,
    address,
...
exports.createGeneration = (
  ...
  if (
    coin.sapling === true ||
    (typeof coin.sapling === "number" && coin.sapling <= rpcData.height)
  ) {
    txb.setVersion(bitcoin.Transaction.ZCASH_SAPLING_VERSION);
  } else if (
    coin.overwinter === true ||
    (typeof coin.overwinter === "number" && coin.overwinter <= rpcData.height)
  ) {
  ...
  if (
    coin.payZelNodes === true ||
    (typeof coin.payZelNodes === "number" &&
      coin.payZelNodes <= Date.now() / 1000)
  ) {
  ...
  let blockHeightSerial =
    (rpcData.height.toString(16).length % 2 === 0 ? "" : "0") +
  ...
  txb.addInput(
    ...
    new Buffer.concat([
      ...
      Buffer(
        poolHex
          ? poolHex
          : "44656661756C7420732D6E6F6D7020706F6F6C2068747470733A2F2F6769746875622E636F6D2F732D6E6F6D702F732D6E6F6D702F77696B692F496E73696768742D706F6F6C2D6C696E6B",
        "hex"
      ),
    ])
  );
  ...
  if (coin.vFundingStreams) {
    ...
    txb.addOutput(
      ...
      Math.round(
        Math.round(
          blockReward.total *
            (1 - (coin.percentFoundersReward + feePercent) / 100) +
            feeReward
        )
      )
    );
    ...
    rpcData.fundingstreams.map((founderItem) => {
      txb.addOutput(
        scriptFoundersCompile(
          bitcoin.address.fromBase58Check(founderItem.address).hash
        ),
  ...
  // TODO: This sorely needs to be updated and simplified
    ...
    (masternodePayment === false || masternodePayment === undefined) &&
    payZelNodeRewards === false &&
    !rpcData.coinbase_required_outputs
  ) {
    // txs with founders reward
    // This section is for ZEN + other coins
    if (
      coin.payFoundersReward === true &&
      (coin.maxFoundersRewardBlockHeight >= rpcData.height ||
        coin.symbol === "zen" ||
        coin.symbol === "zent" ||
        coin.payAllFounders === true)
    ) {
      // This section is for Horizen
      if (coin.symbol === "zen" || coin.symbol === "zent") {
        // select the correct fork for current height
        let forkIndex = coin.forks.findIndex(
          (f) => rpcData.height < f.activationHeight
        );
        ...
        txb.addOutput(
          ...
          Math.round(
            blockReward.total *
              (1 -
                (pctTreasury + pctSecureNodes + pctSuperNodes + feePercent) /
                  100)
          ) + feeReward
        );
        ...
        // treasury reward
        if (pctTreasury) {
          let indexTre = fork.treasury.changeInterval
            ? parseInt(
                Math.floor(
                  ((rpcData.height - fork.activationHeight) /
                    fork.treasury.changeInterval) %
                    fork.treasury.addresses.length
                )
              )
            : 0;
          let treasuryAddrHash = bitcoin.address.fromBase58Check(
            fork.treasury.addresses[indexTre]
          ).hash;
          txb.addOutput(
        ...
        // Secure Nodes reward
        if (pctSecureNodes) {
          let indexSec = fork.securenodes.changeInterval
            ? parseInt(
                Math.floor(
                  ((rpcData.height - fork.activationHeight) /
                    fork.securenodes.changeInterval) %
                    fork.securenodes.addresses.length
                )
              )
            : 0;
          let secureNodesAddrHash = bitcoin.address.fromBase58Check(
            fork.securenodes.addresses[indexSec]
          ).hash;
          txb.addOutput(
        ...
        // Super Nodes reward
        if (pctSuperNodes) {
          let indexSup = fork.supernodes.changeInterval
            ? parseInt(
                Math.floor(
                  ((rpcData.height - fork.activationHeight) /
                    fork.supernodes.changeInterval) %
                    fork.supernodes.addresses.length
                )
              )
            : 0;
          let superNodesAddrHash = bitcoin.address.fromBase58Check(
            fork.supernodes.addresses[indexSup]
          ).hash;
          txb.addOutput(
        ...
        var poolDeductionAmount = Math.round(
          blockReward.total * (poolFeeDeductionTotalPercent / 100)
        );
        ...
        if (rpcData.infrastructure && rpcData.infrastructure > 0) {
          let infrastructureAddrHash = bitcoin.address.fromBase58Check(
            coin.infrastructureAddresses[0]
          ).hash;
          txb.addOutput(
            scriptFoundersCompile(infrastructureAddrHash),
            blockReward.infrastructure
          );
        }
        // Giveaways
        if (rpcData.giveaways && rpcData.giveaways > 0) {
          let giveawaysAddrHash = bitcoin.address.fromBase58Check(
            coin.giveawayAddresses[0]
          ).hash;
          txb.addOutput(
            scriptFoundersCompile(giveawaysAddrHash),
            blockReward.giveaways
          );
        }
        // Add founders
        ...
        let foundersAddrHash = bitcoin.address.fromBase58Check(
          rpcData.coinbasetxn.foundersaddress
        ).hash;
        ...
        // founders reward
        let index = parseInt(
          Math.floor(rpcData.height / coin.foundersRewardAddressChangeInterval)
        );
        let foundersAddrHash = bitcoin.address.fromBase58Check(
          coin.vFoundersRewardAddress[index]
        ).hash;
    ...
    payZelNodeRewards === false &&
    rpcData.coinbase_required_outputs &&
    rpcData.coinbase_required_outputs.length
  ) {
    ...
    rpcData.coinbase_required_outputs.map((output) => {
      ...
      txb.addOutput(new Buffer(output.script, "hex"), output.amount);
    });
    ...
    let masternodeAddrHash = masternodePayee
      ? bitcoin.address.fromBase58Check(masternodePayee).hash
      : null;
    ...
    // start Vidulum
      ...
      coin.VRSEnabled &&
      rpcData.vrsAddress &&
      rpcData.height >= coin.VRSBlock
    ) {
      // Vidulum Reward System
      let vrsAddrHash = bitcoin.address.fromBase58Check(
        rpcData.vrsAddress
      ).hash;
      ...
      txb.addOutput(
        ...
        Math.round(
          blockReward.total *
            (1 - rpcData.vrsReward / blockReward.total - feePercent / 100)
        ) +
          feeReward -
          masternodeReward
      );
    ...
    //end Vidulum
      ...
      if (
        coin.payFoundersReward === true &&
        (coin.maxFoundersRewardBlockHeight >= rpcData.height ||
          coin.treasuryRewardStartBlockHeight)
      ) {
        // founders or treasury reward?
        if (
          coin.treasuryRewardStartBlockHeight &&
          rpcData.height >= coin.treasuryRewardStartBlockHeight
        ) {
          // treasury reward
          let index = parseInt(
            Math.floor(
              ((rpcData.height - coin.treasuryRewardStartBlockHeight) /
                coin.treasuryRewardAddressChangeInterval) %
                coin.vTreasuryRewardAddress.length
            )
          );
          let foundersAddrHash = bitcoin.address.fromBase58Check(
            coin.vTreasuryRewardAddress[index]
          ).hash;
          ...
          txb.addOutput(
            ...
            Math.round(
              blockReward.total *
                (1 - (coin.percentTreasuryReward + feePercent) / 100)
            ) +
              feeReward -
              masternodeReward
          );
          ...
          let index = parseInt(
            Math.floor(
              rpcData.height / coin.foundersRewardAddressChangeInterval
            )
          );
          let foundersAddrHash = bitcoin.address.fromBase58Check(
            coin.vFoundersRewardAddress[index]
        ...
        txb.addOutput(
          scriptCompile(poolAddrHash),
          Math.round(blockReward.total * (1 - feePercent / 100)) +
            feeReward -
            masternodeReward
        );
    ...
    // case for ZelCash
    let zelnodeBasicAddrHash = zelnodeBasicAddress
      ? bitcoin.address.fromBase58Check(zelnodeBasicAddress).hash
      : null;
    let zelnodeSuperAddrHash = zelnodeSuperAddress
      ? bitcoin.address.fromBase58Check(zelnodeSuperAddress).hash
      : null;
    let zelnodeBamfAddrHash = zelnodeBamfAddress
      ? bitcoin.address.fromBase58Check(zelnodeBamfAddress).hash
      : null;
