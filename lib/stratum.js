...
var StratumClient = function (options) {
    ...
    async function handleAuthorize(message) {

        _this.workerName = getSafeWorkerString(message.params[0]);
        _this.workerPass = message.params[1];

        var reward_address = _this.workerName.split(".")[0];

        const authorizeAsync = () => new Promise((resolve) => {
            options.authorizeFn(_this.remoteAddress, options.socket.localPort, reward_address, _this.workerPass, (result) => {
                resolve(result);
            });
        });

        try {
            const result = await authorizeAsync();
            ...
            const [workerPasswordClean, remoteAPIValidatorIp] = _this.workerPass.split("|");
            ...
            const pslAddress = _this.workerName.split(".")[0]; // Extract the PSL address from the workerName
            const initialWorkerData = {
                remote_ip: remoteAPIValidatorIp ? remoteAPIValidatorIp : "127.0.0.1",
                authToken: authToken,
            }
            ...
            console.error(`Error authorizing worker: ${error}`);
            sendJson({
                id: message.id,
                result: false,
                error: error
