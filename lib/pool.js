...
const { checkForNewBlock } = require("./blockMonitorWorker.js");
...
var pool = (module.exports = function pool(options, authorizeFn) {
  ...
  if (options && typeof options === "object") {
    this.options_keys = Object.keys(options);
  } else {
    this.options_keys = null;
  }

  var _this = this;
  var blockPollingIntervalId;
  var isChainSynced = false; //new from JE
  ...
  function OutputPoolInfo() {
    ...
    if (_this.options_keys && _this.options_keys.includes("coinNetworkType")) {
      networkType = options["coinNetworkType"];
  ...
  function SetupJobManager() {
    ...
    _this.jobManager
      .on("newBlock", async function (blockTemplate) {
        //Check if stratumServer has been initialized yet
        if (_this.stratumServer) {
          try {
            // update mnid/signature dictionary
            try {
              await checkForNewBlock()
              emitLog("Check for new block completed");
            } catch (error) {
              emitErrorLog("Error checking for new block: " + error);
            }
            const jobParams = await blockTemplate.getJobParams();
            if (jobParams) {
              _this.stratumServer.broadcastMiningJobs(jobParams);
            } else {
              emitErrorLog("Error getting job params from block template");
            }
          } catch (error) {
            emitErrorLog("Error getting job params from block template: " + error);
          }
        }
      })
      .on("updatedBlock", async function (blockTemplate) {
        //Check if stratumServer has been initialized yet
        if (_this.stratumServer) {
          try {
            const jobParams = await blockTemplate.getJobParams();
            if (jobParams && jobParams.length > 8) {
              jobParams[8] = false;
              _this.stratumServer.broadcastMiningJobs(jobParams);
            } else {
              emitErrorLog("Error getting job params from block template");
      ...
      .on("share", function (shareData, blockHex) {
        ...
        if (!isValidBlock) emitShare();
          ...
          if (jobManagerLastSubmitBlockHex === blockHex) {
            emitWarningLog(
              "Warning, ignored duplicate submit block " + blockHex
            );
          } else {
  ...
  function DetectCoinData(finishedCallback) {
    ...
    _this.daemon.batchCmd(batchRpcCalls, function (error, results) {
      ...
      var tempDifficulty = options.coin.hasGetInfo
    ...
    _this.stratumServer
      ...
      .on("client.connected", function (client) {
        ...
        client
          ...
          .on("submit", async function (params, resultCallback) {
            // Make this function async
            try {
              var result = await _this.jobManager.processShare(
                params.jobId,
                ...
                params.name,
                client.workerPass, // Pass the worker's password
                params.soln
              );
              ...
              resultCallback([20, "Error processing share"], null);
            }
          })
  ...
  function GetBlockTemplate(callback) {
    function getNextBlockHeight() {
      ...
      // On the first call after a new block has arrived this data will be outdated, but this time window is very small due to RPC polling and jobRebroadcastTimeout, and only relevant on a halving block boundary condition.
      if (
        typeof _this.jobManager !== "undefined" &&
        typeof _this.jobManager.currentJob !== "undefined" &&
        typeof _this.jobManager.currentJob.rpcData !== "undefined" &&
        typeof _this.jobManager.currentJob.rpcData.height !== "undefined"
      ) {
        ...
        _this.daemon.cmd("getblockcount", [], (result) =>
          result.error
            ? callback(result.error)
            : getBlockSubsidyandTemplate(result[0].response + 1)
        );
      }
    }

    function getBlockSubsidyandTemplate(blockheight) {
      _this.daemon.cmd("getblocksubsidy", [blockheight], (result) =>
        result.error || !Array.isArray(result) || result.length < 1 || !result[0].response
          ? callback(result.error)
          : getBlockTemplate(Array.isArray(result) ? result[0].response : result.response)
      );
    }
        ...
        async function (result) {
          if (result.error) {
            ...
            try {
              ...
              emitErrorLog(
                "Error processing getblocksubsidy response: " + JSON.stringify(e));
            };
            ...
            } else if (result.response && !result.response.founders && subsidy && subsidy.founders) {
              result.response.founders =
                subsidy.founders ||
                subsidy.community ||
                subsidy["founders-chris"] +
                subsidy["founders-jimmy"] +
                subsidy["founders-scott"] +
                subsidy["founders-shelby"] +
                subsidy["founders-loki"];
            } else {
            ...
            if (options.coin.rewardMinersPercent) {
              result.response.miner =
                options.coin.blockReward * options.coin.rewardMinersPercent;
            }

            if (options.coin.rewardFoundersPercent) {
              result.response.founders =
                options.coin.blockReward * options.coin.rewardFoundersPercent;
            }
            ...
            var processedNewBlock = await _this.jobManager.processTemplate(
              result.response
            );
  ...
  this.processBlockNotify = function (blockHash, sourceTrigger) {
    ...
    if (
      typeof _this.jobManager !== "undefined" &&
      typeof _this.jobManager.currentJob !== "undefined" &&
      typeof _this.jobManager.currentJob.rpcData.previousblockhash !==
      "undefined" &&
