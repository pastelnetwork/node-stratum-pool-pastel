...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function jsGetPreviousBlockMerkleRoot(callback) {
        if (!isChainSynced) {
            callback(new Error("Blockchain is not fully synced"), null);
            return;
        }            
        // Get current block height
        _this.daemon.cmd('getblockcount', [], function (result) {
            if (result.error) {
                callback(result.error, null);
                return;
            }
            const currentBlockHeight = result.response;
            const previousBlockHeight = currentBlockHeight - 1;
    
            // Get previous block hash
            _this.daemon.cmd('getblockhash', [previousBlockHeight], function (result) {
                if (result.error) {
                    callback(result.error, null);
                    return;
                }
                const previousBlockHash = result.response;
    
                // Get previous block details
                _this.daemon.cmd('getblock', [previousBlockHash], function (result) {
                    if (result.error) {
                        callback(result.error, null);
                        return;
                    }
                    const previousBlockMerkleRoot = result.response.merkleroot;
                    callback(null, previousBlockMerkleRoot);
                });
            });
        });
    }    
    ...
    function OnBlockchainSynced(syncedCallback) {
        ...
        // JE: Replaced this with direct call to local python REST API, keeping here for reference
        // Assuming jobManager is an instance of JobManager and poolInstance is your pool instance
        // _this.jobManager.on('requestPreviousBlockMerkleRoot', function(callback) {
        //     _this.jsGetPreviousBlockMerkleRoot((err, merkleRoot) => {
        //         callback(err, merkleRoot);
        //     });
        // });
        // _this.jsGetPreviousBlockMerkleRoot = jsGetPreviousBlockMerkleRoot;
    }
    ...
    function StartStratumServer(finishedCallback) {
        ...
        _this.stratumServer.on('started', function () {
            ...
            }).on('submit', async function (params, resultCallback) { // Make this function async
                try {
...
// module.exports.jsGetPreviousBlockMerkleRoot = jsGetPreviousBlockMerkleRoot;
