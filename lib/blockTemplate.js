...
this.serializeHeader = async function(nTime, nonce, workerData) {
    ...
    // Correcting the approach to match the original functionality
    Buffer.from(this.prevHashReversed, 'hex').copy(header, position);
    position += 32;
  
    // Merkle root (32 bytes)
    Buffer.from(this.merkleRootReversed, 'hex').copy(header, position);
    position += 32;
  
    // Reserved field (32 bytes)
    Buffer.from(this.hashReserved, 'hex').copy(header, position);
    position += 32;
  
    // Time (4 bytes)
    // Assuming nTime is already a number and directly writing it
    header.writeUInt32LE(nTime, position);
    position += 4;
  
    // Bits, difficulty (4 bytes)
    // Utilizing the original method's approach for consistency
    Buffer.from(this.rpcData.bits, 'hex').reverse().copy(header, position);
    position += 4;
  
    // Nonce (32 bytes)
    Buffer.from(nonce, 'hex').copy(header, position);
    position += 32;
