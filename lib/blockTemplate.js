...
// Initialize synchronous Redis client with specific connection options
const redisClientSync = createClient({
  url: "redis://localhost:6379", // Specify custom Redis instance URL and port
  legacyMode: true, // Enable legacy mode for synchronous operations
});
...
// Function to ensure connection to synchronous Redis client
function connectRedisSync() {
  try {
    redisClientSync.connect();
    console.log("Synchronous Redis client connected.");
  } catch (error) {
    console.error("Failed to connect to synchronous Redis:", error);
    // Optionally, implement retry logic or error handling here
  }
}

// Call connectRedisSync to establish the synchronous connection
connectRedisSync();
...
function getValidPastelIdAndSignature() {
  try {
    // Get all worker keys from Redis synchronously
    const keys = redisClientSync.keys('worker:data:*');
    ...
    const workerDataString = redisClientSync.get(randomKey);
 ...
 */
this.serializeEquihashInput = function(nTime, workerData) {
    ...
    var nDataSize = 4 + 32 + 32 + 32 + 4 + 4 + 
      ...
      signatureLengthInBytes(pastelIdLengthInBytes) + pastelIdLengthInBytes;
   ...
   */
  this.serializeBlockHeader = function (equihashInput, nonce, solution) {
    // block header structure:
    // - header data without nonce and solution
    // - nonce (32 bytes)
    // - solution (variable length)

    const headerBufSize = equihashInput.length + 
      nonce.length / 2 + solution.length / 2;
  ...
  this.getJobParams = function () {
    if (!this.jobParams) {
      const { pastelId, signature } = getValidPastelIdAndSignature(); // Call getValidPastelIdAndSignature to get mnid and prevMerkleRootSignature synchronously     
      this.mnid = pastelId;
      ...
      this.jobParams = [
        ...
        this.mnid,                     // v5: mnid
        this.prevMerkleRootSignature,  // v5: prevMerkleRootSignature
      ];
    }
