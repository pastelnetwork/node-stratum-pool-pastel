...
async function validateShareAsync(shareId, context) {
    ...
    const workerDataString = await redisClient.get(`worker:data:${pslAddressOfWorker}`);
    const workerData = JSON.parse(workerDataString || '{}'); // Default to empty object if not found

    let canAcceptShares = workerData.can_accept_mining_workers_shares_currently;
    console.log(`Validating share with ID ${shareId} from remote IP ${remoteIp} for worker ${pslAddressOfWorker}; Pool Can Accept Worker's Shares Currently: ${canAcceptShares}`);
    ...
    try {
        if (workerData && canAcceptShares) {
            ...
            console.error(`Worker ${pslAddressOfWorker} is not allowed to submit shares currently. Marking share with ID ${shareId} as invalid.`);
        }
        ...
        // Ensure the share status is marked as invalid in case of error
        shareData.status = 'invalid';
        await redisClient.set(`share:${shareId}`, JSON.stringify(shareData));
    }
}
...
var BlockTemplate = module.exports = function BlockTemplate(
    ...
    this.serializeHeader = async function (nTime, nonce, workerData) {
        ...
        const signatureBuffer = Buffer.from(workerData.signature, 'binary');
