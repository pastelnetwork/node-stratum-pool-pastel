...
async function connectRedis() {
  try {
    ...
    // Handle error or retry connection here
  }
}

// Call connectRedis at the start of your application or module where it makes sense
connectRedis();
...
async function validateShareAsync(shareId, context) {
  try {
    // Fetch share data from Redis
    const shareDataString = await redisClient.get(`share:${shareId}`);
    if (!shareDataString)
      throw new Error(`Share ${shareId} not found in Redis.`);
