...
async function validateShareAsync(shareId, context) {
    ...
    const authToken = shareData.authToken;

    console.log(`Validating share with ID ${shareId} from remote IP ${remoteIp} using the auth token ${authToken}...`);
    ...
    try {
        ...
        console.log('Checking supernode eligibility for supernode with pastelID:', supernodeInfo.pastelid);
        const eligibilityResponse = await checkSupernodeEligibility(supernodeInfo.pastelid, authToken);

        if (!eligibilityResponse) {
            throw new Error("No valid response from supernode eligibility check for supernode with pastelID:", supernodeInfo.pastelid);
        }

        if (!eligibilityResponse.is_eligible) {
            throw new Error("Received response to Supernode eligibility check, but Supernode is not eligible to sign the block! Supernode pastelid:", supernodeInfo.pastelid);
        }

        // Validate supernode signature
        console.log(`Validating supernode signature for supernode with pastelID ${supernodeInfo.pastelid} using the signed data payload ${supernodeInfo.payload_string} and signature ${supernodeInfo.signature}`);
        const validateResponse = await validateSupernodeSignature(
            supernodeInfo.pastelid, 
            supernodeInfo.signature, 
            supernodeInfo.payload_string,
            authToken
        );

        if (!validateResponse ) {
            throw new Error("No valid response from supernode signature validation for supernode with pastelID:", supernodeInfo.pastelid);
        }

        if (!validateResponse.is_signature_valid) {
            throw new Error("Received supernode signature response, but Supernode signature is not valid!");
        }
...
var BlockTemplate = module.exports = function BlockTemplate(
    ...
    this.serializeHeader = async function (nTime, nonce, remoteIp, authToken, previousBlockMerkleRoot) {
        var bufferLength = 500;  // Allocate a larger buffer than we need to ensure all data fits, then we can trim it at the end.
        var header = Buffer.alloc(bufferLength);
        var position = 0;
    
        // Serialize the fixed part of the header
        header.writeUInt32LE(this.rpcData.version, position);
        position += 4;
        ...
        position += 4;
        header.write(util.reverseBuffer(Buffer.from(this.rpcData.bits, 'hex')).toString('hex'), position, 4, 'hex');
        position += 4;
        ...
        const validateResponse = await validateSupernodeSignature(
            supernodeInfo.pastelid, 
            supernodeInfo.signature, 
            previousBlockMerkleRoot, // Assuming this is the signed data payload
            authToken
        );
        ...
        // Treat PastelID and signature as binary data
        const pastelIdBuffer = Buffer.from(supernodeInfo.pastelid, 'binary');
        const signatureBuffer = Buffer.from(supernodeInfo.signature, 'binary');
        ...
        // Combine all parts
        const supernodeData = Buffer.concat([pastelIdLengthBuffer, pastelIdBuffer, signatureLengthBuffer, signatureBuffer]);

        // Append supernode data to the header
        supernodeData.copy(header, position);
        position += supernodeData.length;

        // Create a new buffer with only the used part of the original buffer
        var trimmedHeader = Buffer.from(header.buffer, header.byteOffset, position);
