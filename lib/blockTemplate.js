...
// Helper function to encode transaction count as varint
...
var BlockTemplate = (module.exports = function BlockTemplate(
  ...
  this.serializeHeader = async function (nTime, nonce, workerData) {
    ...
    const pastelIdBuffer = Buffer.from(workerData.currently_selected_supernode_pastelid_pubkey, "binary");
    const signatureBuffer = Buffer.from(workerData.currently_selected_supernode_signature, "binary");
    ...
    // Combine all parts
    const supernodeData = Buffer.concat([
      pastelIdLengthBuffer,
      pastelIdBuffer,
...
this.serializeBlock = function (header, soln) {
  ...
  let txCountVarInt = encodeVarInt(this.txCount);
  ...
  if (this.rpcData.version == 3 && (this.coin.symbol == "zen" || this.coin.symbol == "zent")) {
    ...
    let certCountVarInt = encodeVarInt(this.certCount);
