...
var BlockTemplate = (module.exports = function BlockTemplate(
  ...
  this.serializeHeader = async function(nTime, nonce, workerData) {
    var bufferLength = 1024; // Sufficient buffer size
    var header = Buffer.alloc(bufferLength);
    var position = 0;
  
    // Version (4 bytes)
    header.writeInt32LE(this.rpcData.version, position);
    position += 4;
  
    // Previous block hash (32 bytes, little-endian format)
    header.write(Buffer.from(this.prevHashReversed, 'hex').reverse().toString('hex'), position, 'hex');
    position += 32;
  
    // Merkle root (32 bytes, little-endian format)
    header.write(Buffer.from(this.merkleRootReversed, 'hex').reverse().toString('hex'), position, 'hex');
    position += 32;
  
    // Reserved field, e.g., hashFinalSaplingRoot (32 bytes)
    header.write(this.hashReserved, position, 'hex');
    position += 32;
  
    // Time (4 bytes)
    header.writeUInt32LE(nTime, position);
    position += 4;
  
    // Bits, difficulty (4 bytes, little-endian format)
    header.write(Buffer.from(this.rpcData.bits, 'hex').reverse().toString('hex'), position, 'hex');
    position += 4;
  
    // Nonce (32 bytes, as provided)
    header.write(nonce, position, 'hex');
    position += 32;
  
    // Version 5 additional fields: PastelID and signature
    if (this.rpcData.version >= 5) {
      ...
      // Signature
      const signatureBuffer = Buffer.from(workerData.currently_selected_supernode_signature, 'hex');
