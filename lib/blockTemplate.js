...
async function validateShareAsync(shareData, shareId, context, blockHex, blockHash) {
  ...
  try {
    ...
    if (workerData && canAcceptShares) {
      ...
      shareData.blockHash = blockHash;
      shareData.blockHex = blockHex;
 ...
 */
this.serializeHeaderData = function(nTime, workerData) {
    ...
    v4data.writeUInt32LE(bitsAsInt, pos);
    pos += 4;
  ...
  this.serializeEquihashInput = function (headerDataParts, nonce) {
    ...
    // - nonce (32 bytes)
    const bufSize = headerDataParts.v4data.length + nonce.length / 2;
