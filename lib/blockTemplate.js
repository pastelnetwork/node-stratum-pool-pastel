...
var BlockTemplate = (module.exports = function BlockTemplate(
  ...
  if (rpcData.finalsaplingroothash) {
    this.hashFinalSaplingRootReversed = util
      .reverseBuffer(Buffer.from(rpcData.finalsaplingroothash, "hex"))
      .toString("hex");
  } else {
    this.hashFinalSaplingRootReversed =
...
this.serializeHeaderData = async function(nTime, workerData) {
    ...
    pos += writeCompactSize(bufLen, v4data, pos);
    ...
    bufLen = this.hashFinalSaplingRootReversed.length / 2;
    pos += writeCompactSize(bufLen, v4data, pos);
    Buffer.from(this.hashFinalSaplingRootReversed, 'hex').copy(v4data, pos);
    pos += bufLen;

    // Time (4 bytes)
    v4data.writeUInt32LE(parseInt(nTime, 16), pos); // Correctly parsing nTime as hex
    pos += 4;

    // Bits, difficulty (4 bytes)
    Buffer.from(this.rpcData.bits, 'hex').reverse().copy(v4data, pos);
    ...
    if (this.rpcData.version >= 5) {
      pos = 0;
      const v5BufferSize = 
        getCompactSizeLength(workerData.currently_selected_supernode_pastelid_pubkey.length) +
        workerData.currently_selected_supernode_pastelid_pubkey.length + 
        getCompactSizeLength(workerData.currently_selected_supernode_signature.length) + 
        workerData.currently_selected_supernode_signature.length;
      ...
      pos += writeCompactSize(workerData.currently_selected_supernode_pastelid_pubkey.length, v5data, pos);
      ...
      pos += pastelIdBuffer.length;
        ...
        pos += writeCompactSize(signatureBuffer.length, v5data, pos);
