...
async function getPreviousBlockMerkleRoot() {
    try {
        const { previousBlockMerkleRoot } = await blockchainOps.getPreviousBlockHashAndMerkleRoot();
        return previousBlockMerkleRoot;
    } catch (error) {
        ...
        for (const key of keys) {
            ...
            if (signaturePack.previous_block_merkle_root === previousBlockMerkleRoot) {
                ...
                for (const { pastelId, signature } of signaturesArray) {
                    const isValid = await validateSupernodeSignature(pastelId, signature, previousBlockMerkleRoot);
