...
async function validateSupernodeSignature(
    ...
    const signedDataPayloadAsBytesBase64Encoded = Buffer.from(signedDataPayload, 'hex').toString('base64');
    try {
        const verificationResult = await blockchainOps.verifyBase64MessageWithPastelid(supernodePastelIdPubKey, signedDataPayloadAsBytesBase64Encoded, supernodeSignature);
        ...
        for (const key of keys) {
            ...
            if (signaturePack.best_block_merkle_root === bestBlockMerkleRoot) {
                ...
                for (const { pastelId, signature } of signaturesArray) {
                    const isValid = await validateSupernodeSignature(pastelId, signature, signaturePack.best_block_merkle_root);
