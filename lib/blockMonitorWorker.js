...
const fs = require("fs").promises; // Correct import for using promise-based functions
...
(async () => {
  try {
    ...
    // Delay the initial call to refreshGlobalDictionary by 10 seconds (10000 milliseconds)
    await delayExecution(10000, refreshGlobalDictionary);
  } catch (error) {
...
async function refreshGlobalDictionary() {
  ...
  try {
    // Use the promise-based readFile method with await
    const envContent = await fs.readFile(
      "/home/ubuntu/mining_block_supernode_validator/.env",
      "utf8"
    );
    const authTokenLine = envContent
      .split("\n")
  ...
  try {
    ...
    console.log(
      `Found ${keys.length} keys in the global worker dictionary! Processing...`
    );
    ...
    for (const key of keys) {
      ...
      console.log(
        `Refreshing worker data for worker with data: ${workerDataString}`
      );

      const signaturePack = await getSignaturePackFromRemoteMachine(
        workerData.remote_ip,
        workerData.authToken
      );
      if (!signaturePack) {
        console.log(
          `No signature pack found for worker ${workerData.remote_ip}, skipping.`
        );
        continue; // Skip if no signature pack is fetched
      }

      if (
        signaturePack.previous_block_merkle_root === previousBlockMerkleRoot
      ) {
        console.log(
          `Signature pack for worker ${workerData.remote_ip} is up-to-date and matches the previous block Merkle Root determined by the mining pool.`
        );
        let signaturesArray = Object.entries(signaturePack.signatures).map(
          ([pastelId, details]) => ({ pastelId, ...details })
        );
        console.log(
          `Found ${signaturesArray.length} signatures for worker ${workerData.remote_ip}; shuffling these and then validating them.`
        );
        ...
        for (const { pastelId, signature } of signaturesArray) {
          const isValid = await validateSupernodeSignature(
            pastelId,
            signature,
          ...
          if (isValid) {
            console.log(
              `Valid signature found for worker ${workerData.remote_ip} from supernode with PastelID ${pastelId}.`
            );
        ...
        if (!validSignatureFound) {
          console.log(
            `No valid signatures found for worker ${workerData.remote_ip}.`
          );
          workerData.can_accept_mining_workers_shares_currently = false;
        }
      } else {
        console.log(
          `Signature pack for worker ${workerData.remote_ip} is outdated or does not match the previous block Merkle Root determined by the mining pool.`
        );
        workerData.can_accept_mining_workers_shares_currently = false;
      }
      console.log(
        `Updated worker data for worker with data: ${JSON.stringify(
          workerData
        )}`
      );
    ...
    console.log(
      "Global worker dictionary updates refreshed and saved to Redis."
    );
  } catch (error) {
