...
async function validateSupernodeSignature(
    ...
    try {
        const verificationResult = await blockchainOps.verifyMessageWithPastelid(supernodePastelIdPubKey, signedDataPayload, supernodeSignature);
...
async function refreshGlobalDictionary() {
    ...
    try {
        ...
        const keysRandomizedOrders = shuffleArray(keys);
        
        for (const key of keysRandomizedOrders) {
            const workerDataString = await redisClient.get(key);
            if (!workerDataString) {
                console.log(`No data found for key ${key}, skipping.`);
                continue;
