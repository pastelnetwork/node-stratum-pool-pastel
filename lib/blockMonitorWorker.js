...
async function loadPastelBlockchainOperations(retryCount = 3) {
    ...
    while (attempts < retryCount) {
        try {
            ...
            await ops.initialize(); // Make sure to await the initialization
            if (ops.isInitialized) { // Check if the class is fully ready
                console.log("PastelBlockchainOperations loaded and initialized");
                blockchainOps = ops; // Assign to the outer scope variable
